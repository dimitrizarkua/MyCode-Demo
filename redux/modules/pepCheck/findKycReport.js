...
export type FindKycReportParams = {
  fullName: string
} | {
  address: string
}

type FindKycReportResult = Array<{
  "name": string,
  "citizenship": string,
  "residence": string,
...
function getMock(/* params: GetKycReportParams */): FindKycReportResult {
  return [
    {
...
function createUrl(params: FindKycReportParams) {
  return 'admin/kycSearchReport/?' + qs.stringify(params);
}

function createBody(/* params: FindKycReportParams */) {
  return;
}

const { reducer: findKycReportResult, action: findKycReportIfNeeded } = registerHttpAction(({
  name: 'FIND_KYC_REPORT',
  getMock,
  method: 'get',
  createUrl,
  createBody
}: RegisterHttpActionParam<FindKycReportResult, FindKycReportParams>));

export type GetKycReportState = ActionState<FindKycReportResult, FindKycReportParams>
